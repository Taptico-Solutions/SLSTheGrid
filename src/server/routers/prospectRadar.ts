...
import {
  INTERNAL_ROLES,
  prospectBuyingStageEnum,
  prospectLeadStatusEnum,
  prospectLeads,
  prospectSignalTypeEnum,
  prospectSignals,
} from "../db/schema";

const leadStatusValues = prospectLeadStatusEnum.enumValues;
const buyingStageValues = prospectBuyingStageEnum.enumValues;
const signalTypeValues = prospectSignalTypeEnum.enumValues;
...
const ProspectLeadInput = z.object({
  ...
  expectedBidDate: z.string().date().optional(),
  expectedAwardDate: z.string().date().optional(),
  constructionStartDate: z.string().date().optional(),
...
const demoProspects = [
  {
    lead: {
      ...
      primarySignal: "Permit activity and food hall renovation scope indicate decorative architectural lighting package needs.",
    ...
    signals: [
      {
        ...
        description: "Permit language suggests a multi-tenant food hall build-out with updated electrical and finish scopes.",
      ...
      },
      {
        ...
        description: "Early pricing package appears likely within 30–45 days based on timeline and contractor activity.",
      ...
      },
      {
        ...
        description: "Historic shell plus hospitality use case increases need for decorative, architectural, and code-compliant lighting coordination.",
  ...
  },
  {
    lead: {
      ...
      primarySignal: "Healthcare expansion planning points to exam-room, corridor, exterior, and controls package opportunity.",
      sourceName: "Illustrative market radar seed",
      summary:
    ...
    signals: [
      {
        ...
        description: "Public market chatter indicates ambulatory expansion planning and capital allocation.",
      ...
      },
      {
        ...
        description: "Healthcare design partner appears aligned with project type and could influence specification early.",
  ...
  },
  {
    lead: {
      ...
      primarySignal: "GC award and accelerated bid timing signal an immediate high-bay and site-lighting package opportunity.",
      sourceName: "Illustrative market radar seed",
      summary:
    ...
    signals: [
      {
        ...
        description: "Named GC indicates the project has moved from planning toward active procurement and bid coordination.",
      ...
      },
      {
        ...
        description: "Construction start timing creates urgency for lighting submittals, long-lead fixture strategy, and controls coordination.",
  ...
  },
  {
    lead: {
      ...
      primarySignal: "Municipal modernization and bid preparation indicate auditorium, exterior, and public-space lighting needs.",
      sourceName: "Illustrative market radar seed",
      summary:
    ...
    signals: [
      {
        ...
        description: "Bid timing appears to be moving into pricing preparation, with a likely public procurement process.",
      ...
      },
      {
        ...
        description: "Project scope aligns with funded public-facility modernization work.",
...
export const prospectRadarRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const filters = [
        ...
        input?.minHeatScore !== undefined ? gte(prospectLeads.heatScore, input.minHeatScore) : undefined,
      ].filter(Boolean);

      const rows = await ctx.db.query.prospectLeads.findMany({
        where: filters.length ? and(...(filters as any)) : undefined,
        with: { signals: true, assignedRep: true },
  ...
  get: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      requireInternal(ctx.user.role);
      const row = await ctx.db.query.prospectLeads.findFirst({
        where: eq(prospectLeads.id, input.id),
        with: { signals: true, assignedRep: true, creator: true },
      });
  ...
  create: protectedProcedure.input(ProspectLeadInput).mutation(async ({ ctx, input }) => {
    ...
    const [row] = await ctx.db
      ...
      .values({
        ...input,
        estimatedProjectValue: money(input.estimatedProjectValue),
        estimatedLightingValue: money(input.estimatedLightingValue),
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [row] = await ctx.db
        ...
        .set({
          ...rest,
          ...(estimatedProjectValue !== undefined ? { estimatedProjectValue: money(estimatedProjectValue) } : {}),
          ...(estimatedLightingValue !== undefined ? { estimatedLightingValue: money(estimatedLightingValue) } : {}),
          updatedAt: new Date(),
        })
  ...
  addSignal: protectedProcedure.input(ProspectSignalInput).mutation(async ({ ctx, input }) => {
    requireInternal(ctx.user.role);
    const [lead] = await ctx.db.select({ id: prospectLeads.id }).from(prospectLeads).where(eq(prospectLeads.id, input.prospectId));
    if (!lead) throw new TRPCError({ code: "NOT_FOUND" });
    const [row] = await ctx.db.insert(prospectSignals).values(input).returning();
    await ctx.db.update(prospectLeads).set({ updatedAt: new Date() }).where(eq(prospectLeads.id, input.prospectId));
    return row;
  }),
  ...
  loadDemo: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const existing = await ctx.db
      ...
      .from(prospectLeads)
      .where(inArray(prospectLeads.projectName, demoProspects.map((p) => p.lead.projectName)));
    ...
    for (const prospect of demoProspects) {
      ...
      await ctx.db.insert(prospectSignals).values(
